...
% Last Modified by GUIDE v2.5 29-May-2020 00:35:15
...
% BLOQUE DE CÓDIGO DE HARDCODEO DE CHABELO
function setgcurve1()
global gcurve1
...
Fs = getFs();
nBits = getnBits(); 
setRecordingTime(get(handles.slider2,'Value')); %corrección de bug de inicio
...
% Distance in cents
set(handles.text21,'String', distance);
if distance > 0
...
% hObject    handle to applyFilters (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
[band, DF]=bandpass(signal,[2 80], 1000, 'ImpulseResponse', 'fir');
[signal, DF]=highpass(signal,350, 1000, 'ImpulseResponse', 'fir');
...
[signal, DF]=highpass(signal,20, 1000, 'ImpulseResponse', 'fir','Steepness', 0.95);
...
[signal, DF] = bandpass(signal,[f1 f2], 1250, 'Steepness', 0.95, 'ImpulseResponse', 'fir');
...
[signal, DF]=bandpass(signal,[f3 f4], 1250, 'Steepness', 0.95, 'ImpulseResponse', 'fir');
...
% l3=[ 'The covariance is: ' int2str(rr) '  ' ];
...
% 
% l6= ['Normalized covariance: ' normalize(rr)];
uiwait(msgbox({l1; l2; } ,'Random processes !'));
